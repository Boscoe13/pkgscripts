...
def updateProject(projects, target):
	proj_list = ''
	for proj in projects:
		if proj == 'uistring' and os.path.isdir(BaseDir+'/source/'+proj+'/.git'):
			try: check_call('cd '+BaseDir+'/source/'+proj+'; git reset --hard; git pull --rebase', shell=True)
			except CalledProcessError: pass
			continue
		if proj == VAR_KERNEL_PROJ: continue
...
class UpdateHook(TraverseHook):
	def perform(self, config):
		updateProject(config['proj']['curr'], self.branch)
		if self.do_base: updateProject(config['proj']['base'], self.arch+':'+config['base'])
...
def prepareProjects(arch):
	cmd = ScriptDir+'/ProjDepends.py -p '+arch+' '+ProjDependsOpt+' '+string.join(InputProjects, ' ')
	...
	if DoLink:
		for proj in ['/source/'+p for p in (projects | extra)]+['/pkgscripts']:
...
def buildPackage(projects, build_opt):
	global PkgError
	build_cmd = 'chroot . /pkgscripts/SynoBuild '
	plist = []
	curr_dir = os.getcwd()
	for arch in Platforms:
		...
		build_opt += ' --min-sdk ' + getEnvVer(arch, DictEnv)
		log_file = 'logs.build'
...
def signPackage(arch, pattern):
	global PkgError
	major_ver, minor_ver = getEnvVer(arch, DictEnv).split(".")
...
def collectPackage(package, do_sign, milestone=None):
	...
	if os.path.exists(dst_dir):
		os.rename(dst_dir, dst_dir+'.bad.'+strftime('%Y%m%d-%H%M', localtime()))
	os.makedirs(dst_dir)
	...
	if not (DoUpload and checkBuildMachine(ScriptDir+'/include/toolkit.config')): return
...
if __name__ == '__main__':
	...
	if DoUpdate or DoLink or DoBuild:
		...
		for arch in Platforms:
			...
			DictVar, DictDep, DictKernel = readDependsBase(ScriptDir+'/include/project.depends')
			DictInfo = {'env': DictEnv, 'var': DictVar, 'dep': DictDep, 'kernel': DictKernel}
			Seen, RefOnly, ForPack = traverseSource(InputProjects, BaseDir, arch, DictInfo, hook, DoBase)
		...
		DateBuild0 = time()
		if DoBuild: buildPackage(ProjectList, SynoBuildOpt)
